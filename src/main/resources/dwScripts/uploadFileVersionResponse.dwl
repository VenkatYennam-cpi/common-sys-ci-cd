...
---
{
    ...
    (name: payload.entries[0].name) if(sizeOf(payload.entries) > 0),
	(size: payload.entries[0].size) if(sizeOf(payload.entries) > 0),
    (etag: payload.entries[0].etag) if(sizeOf(payload.entries) > 0),
	(created_at: payload.entries[0].created_at) if(sizeOf(payload.entries) > 0),
    (modified_at: payload.entries[0].modified_at) if(sizeOf(payload.entries) > 0)
}